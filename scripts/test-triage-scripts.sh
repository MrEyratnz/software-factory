#!/usr/bin/env bash
# Regression tests for the triage wrapper scripts (triage-add-labels.sh,
# triage-comment.sh). Those scripts are the tool-layer enforcement boundary
# for the Issue Triage workflow — the only thing preventing a prompt-injected
# triage session from retargeting another issue/PR, editing titles/bodies, or
# removing labels — so their argument contract is CI-gated here: a stub `gh`
# on PATH records what would be executed, and each case asserts pass-through
# or rejection. Dependency-free, mirrors tests/hooks.contract.test.sh in
# spirit (hermetic, stubbed gh, no network).
set -euo pipefail
