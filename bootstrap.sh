...
"Layer 2 of docs/specs/epic-1/spec.md: stdin JSON fixtures per event type, exit-code + stderr class-tag assertions ([hard-boundary] vs [heuristic]), matcher edge cases, forgery-guard cases, multi-repo -C/cd binding; coverage >=95% lines over exactly the scope Release-Gate criterion 7 names (docs/specs/epic-1/spec.md § \"Release Gate for v1.0.0\" — the spec governs on drift; title kept verbatim for ensure_issue title-match idempotency, its historical phrasing is narrower than the criterion-7 scope) enforced as a failing test. Owner field: coder."
